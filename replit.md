...
- **Admin Tools:** Comprehensive admin panel with sidebar navigation (desktop: vertical sidebar on right with 4 grouped sections — Administration, Content, Analytics, Settings; mobile: Sheet/drawer menu via hamburger button). 12 tabs: tickets, users, reports, reviews, essays, memoires, social, analytics, API usage, tracking, promos, features. Includes user management, content moderation, revenue analytics, API usage tracking, platform review moderation (pending/all filter), content report management (with direct "View Content" button), memoire analytics (views/clicks/CTR with university/field/methodology/country columns), promo code management (create/toggle/delete), and a robust feature toggle system.
...
- **Memoire Gallery Page**: Dedicated public page at `/memoires` showcasing shared academic memoires with field filters, search, university/country/methodology badges, keyword chips, author links, and SEO structured data (Thesis schema). API: `GET /api/public/memoires`.
