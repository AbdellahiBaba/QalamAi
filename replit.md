...
- **Content Type Specialization:** Each content type is tailored with specific AI personas and input fields. Reels (short-form video scripts) is a sub-type of social_media with genre="reels", supporting Instagram Reels, TikTok, and YouTube Shorts with shot-by-shot breakdowns, visual cues, music suggestions, and cross-platform optimization. This includes an advanced Arabic Classical Poetry module with a full prosody engine and an Academic Memoire module offering deep specialization per academic major and country-specific university rules, featuring a comprehensive university database, faculty-aware methodology filtering, and faculty-aware citation style filtering. The `mapFacultyToField()` function maps Arabic faculty names to academic domain keys, and `getMethodologiesByFaculty()`/`getCitationStylesByFaculty()` use faculty context to filter dropdown options dynamically.
