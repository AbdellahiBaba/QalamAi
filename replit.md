...
- Admin Feature Toggle System: Admin can activate/deactivate platform features via "المميزات" tab. Uses `platformFeatures` table with `featureKey`, `name`, `description`, `enabled`, `betaUserIds` (array), `disabledMessage`. `useFeatures()` hook (`client/src/hooks/use-features.ts`) fetches `/api/features`. Disabled features show "قريباً" badge in dashboard dropdown. Server enforces feature toggles on `POST /api/projects`. Beta users (in `betaUserIds` array) can access disabled features. 12 default features seeded on startup.
