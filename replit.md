...
- Arabic Classical Poetry (الشعر العمودي): Seventh content type "poetry" with full prosody engine. `server/arabic-prosody.ts` encodes all 15 بحور (meters) with تفعيلات, زحافات, علل, 5 literary eras, 14 themes, 10 tones, 28 rhyme letters, and verse count options. `buildPoetryPrompt()` in `server/abu-hashim.ts` constructs prosody-aware prompts for GPT-5.2. Flow is like khawater: single auto-chapter "القصيدة", no outline. Schema fields: `poetryMeter`, `poetryRhyme`, `poetryEra`, `poetryTone`, `poetryTheme`, `poetryVerseCount`, `poetryImageryLevel`, `poetryEmotionLevel`. Price: $49.99 (4999). Poetry creation UI in new-project.tsx with meter selector grid, rhyme letter picker, era/tone/theme dropdowns, imagery/emotion sliders. Project-detail.tsx displays verses with صدر/عجز split formatting. API endpoint `GET /api/poetry/prosody-data` provides metadata.
