...
- Legal Pages: Privacy policy (`/privacy`), Terms of Service (`/terms`), and Refund Policy (`/refund`) — all Arabic static content pages following about.tsx pattern. Linked from SharedFooter "المزيد" column. Routes registered in both PublicRouter and AuthenticatedRouter.
- Project-Detail Mobile Layout: On screens <640px, action buttons (export, share, publish switches, shortcuts, delete) collapse into a DropdownMenu triggered by MoreVertical icon. On sm+ screens, inline buttons display as before. ThemeToggle remains visible on all screen sizes.
