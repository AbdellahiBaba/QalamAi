...
- **Content Type Specialization:** Each content type is tailored with specific AI personas and input fields. Reels (short-form video scripts) is a sub-type of social_media with genre="reels", supporting Instagram Reels, TikTok, and YouTube Shorts with shot-by-shot breakdowns, visual cues, music suggestions, and cross-platform optimization. Reels projects can generate actual MP4 videos via `server/video-generator.ts`: GPT-5.2 parses scripts into scenes, DALL-E generates scene images (1080x1920 portrait), TTS creates Arabic voiceover (voice "onyx"), and FFmpeg stitches everything into a final MP4 with text overlays. Videos are saved to `generated_videos/` and served via Express static middleware. The `videoUrl` column on `novelProjects` stores the path. API: `POST /api/projects/:id/generate-video` (async with polling via `GET /api/projects/:id/video-status`). In-memory job locking prevents concurrent generation per project. This also includes an advanced Arabic Classical Poetry module with a full prosody engine and an Academic Memoire module offering deep specialization per academic major and country-specific university rules, featuring a comprehensive university database, faculty-aware methodology filtering, and faculty-aware citation style filtering. The `mapFacultyToField()` function maps Arabic faculty names to academic domain keys, and `getMethodologiesByFaculty()`/`getCitationStylesByFaculty()` use faculty context to filter dropdown options dynamically.
...
- **FFmpeg**: Video composition for Reels — stitches AI-generated scene images, TTS audio, and text overlays into MP4 files.
