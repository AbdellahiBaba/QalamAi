...
- **Tracking Pixels (TikTok & Facebook)**: Admin-managed pixel tracking via "بكسل التتبع" tab in admin panel. Supports TikTok Pixel + Events API and Facebook Pixel + Conversions API. Browser-side tracking auto-injects pixel scripts for enabled pixels (page views on navigation). Server-side tracking fires Purchase and InitiateCheckout events via `server/tracking.ts`. Access tokens are never exposed to the frontend — admin UI shows `hasAccessToken` indicator. Pixel config stored in `trackingPixels` table. Public API at `GET /api/tracking-pixels` returns only platform + pixelId for enabled pixels. Component: `client/src/components/tracking-pixels.tsx` mounted globally in App.tsx. Pixel cache has 60s TTL, invalidated on admin save.
