...
import { Feather, LogOut, ArrowRight, Send, Loader2, Clock, ShieldCheck, User, AlertCircle, Mail } from "lucide-react";
...
export default function AdminTicket() {
  useDocumentTitle("إدارة التذكرة — قلم AI");
  ...
  const replyMutation = useMutation({
    mutationFn: async (message: string) => {
      const res = await apiRequest("POST", `/api/admin/tickets/${ticketId}/reply`, { message });
      return res.json();
    },
    onSuccess: () => {
      setReplyMessage("");
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: { status?: string; priority?: string }) => {
      const res = await apiRequest("PATCH", `/api/admin/tickets/${ticketId}`, data);
      return res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/tickets", ticketId] });
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <Link href="/admin">
              <Button variant="outline">العودة إلى لوحة الإدارة</Button>
            </Link>
          </div>
        ) : (
          <div className="space-y-8">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-2">
                  <span className="text-sm text-muted-foreground font-mono" data-testid="text-ticket-id">#تذكرة-{ticket.id}</span>
                </div>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {new Date(ticket.createdAt).toLocaleDateString("ar-EG", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
            ...
            {ticket.replies && ticket.replies.length > 0 && (
              <div className="space-y-4">
                <h3 className="font-serif text-lg font-semibold">المحادثة ({ticket.replies.length})</h3>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {reply.isAdmin && (
                              <span className="text-xs px-1.5 py-0.5 rounded bg-primary/10 text-primary">مشرف</span>
                            )}
                            <span className="text-xs text-muted-foreground">
            ...
            <Card>
              <CardContent className="p-6">
                <h3 className="font-serif text-lg font-semibold mb-4">رد المشرف</h3>
                <form onSubmit={handleReply} className="space-y-4">
                  ...
                  />
                  <Button type="submit" disabled={replyMutation.isPending || !replyMessage.trim()} data-testid="button-send-admin-reply">
